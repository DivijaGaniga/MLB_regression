...
# ptb using nls method
  ...
  nls_ptb <- nls(ptb ~ 0.5+i*(R-RA) , data = teams_train, start = list(i =2))
summary(nls_ptb)


 PTB_fit2 <- lm(winPct ~ ptb, data = teams_train)
...
set.seed(267)
testrecs <- createDataPartition(teams_hw3$avgAtt, p=0.5, list = FALSE) 
cs_test <- teams_hw3[testrecs,]
cs_fit <- teams_hw3[-testrecs,] 

fitcontrol <- trainControl(
          method = "CV", number= 5)

fitControl <- trainControl(method = "lm", number =3)
set.seed(120)
#
avg_fit <- train(avgAtt ~ R*HR*E+SOA+FP+RA+H+BB+SB*CS, data = teams_hw3, method = "lm")
avg_fit

attpred <- predict(avg_fit, newdata = teams_hw3)
maeatt_pred <- MAE(attpred, teams_hw3$avgAtt)
maeatt_pred
...
#avgatt_fit1 <- lm(avgAtt ~ R*HR*E+SOA+FP+RA+H+BB+SB*CS, data = teams_train1)

avgatt_fit1 <- lm(avgAtt ~ winPct, data = teams_train)
...
varlist <- names(teams_train1)[1:22]

models <- lapply(varlist, function(x) {
    lm(substitute(avgAtt ~ i, list(i = as.name(x))), data = teams_train1)
})
